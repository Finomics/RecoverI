...
function TransferScreen({ navigation }) {
  ...
  function PaymentTransferCashier() {
    setLoad(true);
    let amounts = [0];
    if (Array.isArray(transferId) && !transferId.length) {
      Alert.alert("Alert", "Please finalise your transfer details");
      setLoad(false);
    } else {
      // setLoad(false);
      // setTransferId("");
  ...
  return (
    <Screen>
      <Header header={"Transfer"} navigation={navigation} />
      <View style={styles.logoContainer}>
        ...
        <View style={styles.textContainer}>
          <TextInput
            style={{ fontWeight: "bold", fontSize: 18, width: "100%" }}
